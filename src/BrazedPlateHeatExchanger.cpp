...
void BrazedPlateHeatExchanger::BPHE_1phase(BPHEInputs BPHE)
{  
    double X = 2*M_PI*BPHE.PlateAmplitude/BPHE.PlateWavelength;
    double PHI = 1.0/6.0*(1+sqrt(1+X*X)+4*sqrt(1+X*X/2));
    
    // The projected surface between the ports
    double A0 = BPHE.Bp*BPHE.Lp;
    
    // The plane surface of one plate
    double Ap = PHI*A0;
    
    // The volume of one channel
    double Vchannel = BPHE.Bp*BPHE.Lp*2*BPHE.PlateAmplitude;
    
    // Hydraulic diameter
    double dh = 4*BPHE.PlateAmplitude/PHI;
    ...
    double w_g = BPHE.mdot_channel/BPHE.CPS.rho()/(2*BPHE.PlateAmplitude*BPHE.Bp);
    ...
    // Pressure drop [Pa]
    double DELTAP = Hg*eta_g*eta_g*BPHE.Lp/(rho_g*dh*dh*dh);
}